...
khge_stride = 2
khge_max_arity = 6
khge_cuda = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
...
if khge_model == "MTransH":
    ...
    if dataset.split("/")[-2] == "M-FB15K":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop = 512, 10, 200, 0.015, 100, 1, 0.2
    if dataset.split("/")[-2] == "FB-AUTO":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop = 256, 10, 200, 0.01, 100, 1, 0.2
    if dataset.split("/")[-2] == "FB15k":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop = 1024, 10, 200, 0.02, 100, 1, 0.2
    if dataset.split("/")[-2] == "wn18":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop = 256, 10, 200, 0.01, 100, 1, 0.2
...
if khge_model == "HypE":
    ...
    if dataset.split("/")[-2] == "M-FB15K":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop, khge_input_drop, khge_in_channels, khge_out_channels, khge_stride, khge_filh, khge_filw = 512, 10, 200, 0.015, 100, 1, 0.2, 0.2, 1, 6, 2, 1, 1
    if dataset.split("/")[-2] == "FB-AUTO":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop, khge_input_drop, khge_in_channels, khge_out_channels, khge_stride, khge_filh, khge_filw = 256, 10, 200, 0.01, 100, 1, 0.2, 0.2, 1, 6, 2, 1, 1
    if dataset.split("/")[-2] == "FB15k":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop, khge_input_drop, khge_in_channels, khge_out_channels, khge_stride, khge_filh, khge_filw = 1024, 10, 200, 0.02, 100, 1, 0.2, 0.2, 1, 6, 2, 1, 1
    if dataset.split("/")[-2] == "wn18":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop, khge_input_drop, khge_in_channels, khge_out_channels, khge_stride, khge_filh, khge_filw = 256, 10, 200, 0.01, 100, 1, 0.2, 0.2, 1, 6, 2, 1, 1
    if dataset.split("/")[-2] == "FB15k-237":
        khge_batch, khge_neg_rate, khge_dim, khge_lr, khge_iters, khge_hbatch, khge_hidden_drop, khge_input_drop, khge_in_channels, khge_out_channels, khge_stride, khge_filh, khge_filw = 1024, 10, 200, 0.02, 100, 1, 0.2, 0.2, 1, 6, 2, 1, 1
...
for k in range(iterations):
    ...
    # Variational E-Step ONLY
    # augment_tuple('{}/annotation.txt'.format(workspace_path), '{}/train.txt'.format(path), '{}/train_augmented.txt'.format(workspace_path), mln_threshold_of_tuple)
    # os.system('cp {}/train_augmented.txt {}/train_augmented.txt'.format(workspace_path, path))
    # evaluate('{}/annotation.txt'.format(workspace_path), '{}/pred_khge_test_raw.txt'.format(workspace_path), '{}/pred_khge_test_fil.txt'.format(workspace_path), '{}/result_khge_mln_raw.txt'.format(workspace_path), '{}/result_khge_mln_fil.txt'.format(workspace_path), weight)