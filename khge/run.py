...
def save_model(model, optimizer, measure, args, itr=0, test_or_valid='test', is_best_model=False):
    ...
    measure_name = '{}_{}_measure_{}itr.json'.format(str(time.time()), test_or_valid, itr) if itr else '{}.json'.format(args.model)
...
def main(args):
    ...
    logging.info("Number of training data points: {}".format(len(dataset.data["train"])))
