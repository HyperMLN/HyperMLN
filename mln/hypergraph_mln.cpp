...
struct Tuple
{
	...
	{
		if (u.max_len < v.max_len) return true;
		else if (u.max_len > v.max_len) return false;
		else
		{
			if (u.r == v.r)
			{
				...
				while (i < u.max_len)
				{
					if (u.e[i] < v.e[i]) return true;
					else if (u.e[i] > v.e[i]) return false;
					else i++;
				}
				return false;
			}
	...
	friend bool operator == (Tuple u, Tuple v)
	{
		if (u.max_len != v.max_len) return false;
		else
		{
			if (u.r == v.r)
			{
	...
	friend bool operator < (Rule u, Rule v)
	{
		if (u.type == v.type)
		{
			if (u.r_hypothesis == v.r_hypothesis)
			{
				...
				{
					if (u.r_premise[k] != v.r_premise[k]) return u.r_premise[k] < v.r_premise[k];
				}
			}
...
{
	cout << id2rel[tuple.r] << " ";
	for (int i = 0; i < tuple.max_len; i++) cout << id2ent[tuple.e[i]] << " ";
	cout << endl;
...
void search_subrelation_rules(int relation, int entities[], int ent_len)
{
	...
	while (r_ < rel2id.size())
	{
		...
		for (int k = 0; k != len; k++)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				if (r2e[r_][k].e[i] >= 0 and r2e[r_][k].e[i] < ent2id.size()) e_len++;
				else break;
			}
...
void search_candicate_rules()
{
	...
	for (iter = candidate_rules.begin(); iter != candidate_rules.end(); iter++) rules.push_back(*iter);
...
double precision_subrelation_rule(Rule rule)
{
	...
	for (int k = 0; k != len; k++)
	{
		...
		for (int i = 0; i < tuples[k].max_len; i++) tuple.e[i] = tuples[k].e[i];
		tuple.r = rh;
		if (check_observed(tuple) == true) p += 1;
		q += 1;
	}
...
double precision_inverse_rule(Rule rule)
{
	...
	{
		if (tuples[k].r != rp) continue;
		for (int i = 0; i < tuples[k].max_len; i++) tuple.e[i] = tuples[k].e[tuples[k].max_len - i - 1];
		tuple.r = rh;
...
void search_hidden_with_inverse(int id, int thread)
{
	...
	for (int k = 0; k != len; k++)
	{
		...
		if (check_observed(tuple) == true) continue;
		tuple.type = 'h';
		tuple.valid = 0;
		sem_wait(&mutex);
...
void read_probability_of_hidden_tuples()
{
	if (probability_file[0] == 0)
	{
		Pair rel_ent_pair;
		tuple2id.clear();
		for (int k = 0; k != entity_size; k++) r2e[k].clear();
		for (int k = 0; k != tuples_size; k++)
		{
			...
			if (tuples[k].type == 'o')
			{
				tuples[k].valid = 1;
				tuples[k].truth = 1;
				for (int i = 0; i < tuples[k].max_len; i++) rel_ent_pair.e[i] = tuples[k].e[i];
	...
	while (inFile.getline(line, 1000))
	{
		...
		token = strtok(thisline, s);
		s_relation = token;
		if (rel2id.count(s_relation) == 0) continue;
		tuple.r = rel2id[s_relation];
		...
		for (int k = 0; k < i - 1; k++)
		{
			...
			else tuple.e[k] = ent2id[s_entity];
		}
	...
	for (int k = 0; k != relation_size; k++) r2e[k].clear();
...
void init_weight()
{
	for (int k = 0; k != rule_size; k++) rules[k].weight = (rand() / double(RAND_MAX) - 0.5) / 100;
}
...
void output_predictions()
{
	...
	for (int k = 0; k != tuples_size; k++)
	{
		...
		int r = tuples[k].r;
		int e[10];
		for (int i = 0; i < tuples[k].max_len; i++) e[i] = tuples[k].e[i];
		double prob = tuples[k].logit;

		ofs << id2rel[r] << "\t";
		for (int i = 0; i < tuples[k].max_len - 1; i++) ofs << id2ent[e[i]] << "\t";
...
void save()
{
	...
	for (int k = 0; k < entity_size; k++) ofs << id2ent[k].c_str() << "\t";

	ofs << endl;
	ofs << "Relation_size: " << relation_size << endl;
	for (int k = 0; k < relation_size; k++) ofs << id2rel[k].c_str() << "\t";
	...
	for (int k = 0; k < tuples_size; k++) {
		...
		ofs << r << "\t";
		for (int i = 0; i < tuples[k].max_len; i++) ofs << e[i] << "\t";
		ofs << type << "\t" << valid << endl;
	}
	...
	for (int k = 0; k < rule_size; k++)
	{
		...
		for (int i = 0; i != int(rules[k].r_premise.size()); i++) ofs << id2rel[rules[k].r_premise[i]].c_str() << "\t";
		ofs << weight << endl;
	}
...
int main(int argc, char **argv)
{
	...
	if ((i = ArgPos((char *)"-thresh-tuple", argc, argv)) > 0) tuple_threshold = atof(argv[i + 1]);
